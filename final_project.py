...
def main():
    # Get user inputs
    t, num_mesh_points, left_boundary, right_boundary, material_props, phi_initial, k_initial, tol, max_iterations = get_user_input()
    ...
    for i in range(num_media):
        ...
        if source_type == "f":
            ...
            material_props.append({
                ...
                'end': end, 
                'sigma_t': sigma_t, 
                'sigma_s_to_sigma_t_ratio': sigma_s_ratio, 
...
def diffusion_eigenvalue_solver_1D(
    ...
    material_props  : list of dict, properties for each medium with keys
                      'start', 'end', 'sigma_t', 'sigma_s_ratio', 'sigma_f_to_sigma_a_ratio', 'nu', 'S'
    ...
    S = np.zeros(new_num_mesh_points)
    ...
    for iteration in range(max_iterations):
        ...
        if np.any(sigma_f > 0):  # Fission source exists
            for i in range(1, new_num_mesh_points - 1):
                fission_source[i] = ((nu[i] * sigma_f[i] + nu[i + 1] * sigma_f[i + 1]) / 2) * Phi[i]

        b_new = fission_source / k + S if np.any(S > 0) else fission_source / k
        ...
        # Solve the linear system using a method like Jacobi
        Phi_new, res = Gauss_Seidel_optimized(lower_diag, main_diag, upper_diag, b_new, Phi, tol, max_iterations)
        residuals.append(res)

        # Update eigenvalue (k) only if fission source exists
        if np.any(sigma_f > 0):
            k_new = np.sum(fission_source) / np.sum(Phi_new * sigma_a * dx)
            if abs(k_new - k) < tol:
                k = k_new
                break
            k = k_new
